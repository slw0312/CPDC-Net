...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.tgt_emb = nn.Embedding(1304, d_model)
        self.pos_emb = PositionalEncoding(d_model)
        self.layers = nn.ModuleList([DecoderLayer() for _ in range(16)])
        self.projection = nn.Linear(d_model, 1304, bias=False).to(device)

    def forward(self, dec_inputs, enc_outputs):
        s=dec_inputs.shape[0]
        l = dec_inputs.shape[1]
        dec_outputs = self.tgt_emb(dec_inputs)
        # print(dec_outputs.shape)
        # print(enc_outputs.shape)
        dec_outputs = torch.cat((enc_outputs, dec_outputs), dim=1)
        # print(dec_outputs.shape)
        ...
        x1 = torch.zeros(s, l+13, 13).to(device)
        x2 = torch.ones(s, 13,l).to(device)
        # print(x2.shape)
        # x3=torch.zeros(s,14,14).to(device)
        # x4=torch.cat((x3,x2),dim=2).to(device)
        y = torch.cat((x2,dec_self_attn_mask), dim=1).to(device)
        # print(y.shape)

        y2 = torch.cat((x1, y), dim=2).to(device)
        # print(y2.shape)
        #y2 = torch.cat((y, y1), dim=1).to(device)
        y2 = y2 != 0.0
        # print(y2.shape)
        #print(y2)
        ...
        for layer in self.layers:
            dec_outputs, dec_self_attn = layer(dec_outputs, y2)

            dec_self_attns.append(dec_self_attn)
            # dec_enc_attns.append(dec_enc_attn)
        dec_outputs = dec_outputs[:, 13:]
        # print(dec_outputs.shape)
