...
def data_iterator(validation=False, test=False, model="CNN"):
    ...
    if validation:
        #feat_path = "./data/clip_pre/" +  "test" + ".pkl"
        feat_path = config1.h5path + "cap4/feat/" + "val" + ".h5"
        logs_path = config1.h5path + "cap4/log/" + "val" + ".h5"
        # pos_path = config1.h5path + "cap/log/" + "posval" + ".h5"
        #caption_path = config1.h5path + "cap/log_action1/" + "test" + ".h5"
        ...
        # pos = h5py.File(pos_path, 'r')
        #cap= h5py.File(caption_path, 'r')
        ...
        #feat_path = "./data/clip_pre/" +  "test" + ".pkl"
        feat_path = config1.h5path + "cap4/feat/" + "test" + ".h5"
        logs_path = config1.h5path + "cap4/log/" + "test" + ".h5"
        # pos_path = config1.h5path + "cap/log/" + "postest" + ".h5"
       #caption_path = config1.h5path + "cap/log_action1/" + "test" + ".h5"
        # with open(feat_path, 'rb') as f:
        #     feat = pickle.load(f)
            # feat=np.array(feat)
        ...
        feat_path = config1.h5path + "cap1/feat/" + "train" + ".h5"
        logs_path = config1.h5path + "cap1/log/" + "train" + ".h5"
        # pos_path = config1.h5path + "cap/log/" + "postrain" + ".h5"
        # caption_path = config1.h5path + "cap/log_action1/" + "test" + ".h5"
        # with open(feat_path, 'rb') as f:
        #     feat = pickle.load(f)
        # feat=np.array(feat)
    ...
    # Datagen
    batch_size = config1.batch_size_gen

    # Load data


   # alphas_batch = np.zeros((batch_size, 10, 240), dtype='float16')
    # context_batch = np.zeros((batch_size, 10, 64), dtype='float16')
    # pred_course_batch = np.zeros((batch_size, 10), dtype='float16')
    # pred_accel_batch = np.zeros((batch_size, 10), dtype='float16')
    course_batch = np.zeros((batch_size, 10), dtype='float32')
    accel_batch = np.zeros((batch_size, 10), dtype='float32')
    captions_batch = np.zeros((batch_size, 1, 22), dtype='int64')
    # pos_batch = np.zeros((batch_size, 1, 22), dtype='int16')
    #feat_batch = np.zeros((batch_size, 10, 64, 12, 20), dtype='float16')
    #img_batch = torch.zeros((batch_size, 10, 512), dtype='float16')
    #img_batch = torch.zeros((batch_size, 10, 512), dtype=torch.float32)
    img_batch = np.zeros((batch_size, 1, 384,640,3), dtype='float32')

    #masks_batch = np.zeros((batch_size, 10), dtype='int16')
    ...
    seq_id_batch = np.zeros((batch_size, 10), dtype='int64')


    try_counter = 0
    if validation==True:
        num_of_batches = int(config1.batches_per_dataset_v)
    elif test==True:
        num_of_batches = int(config1.batches_per_dataset_t)
    else:
        num_of_batches = int(config1.batches_per_dataset)
    ...
    while True:
        try:
            ...
            # sanity check
            assert img_batch.shape == (batch_size, 1, 384,640,3)
            ...
            array = (captions_batch,  goaldir_batch, speed_batch, timestamp_batch, seq_id_batch, course_batch, accel_batch, img_batch)
