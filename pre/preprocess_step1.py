...
def main():
    ...
    config = dict2(**{
        ...
        "annotations":  './data/BDD-X-Dataset/Sample.csv',
        "save_path":    './data/processed2/',
        "data_path":    '../causal-natural-language-explanations-main/bdd100k_new/',
        "chunksize":    20})
        ...
        for item in examples:
            ...
            if (os.path.isfile(config.save_path+"cam/" + str(videoid) + "_" + str(vidName) + ".h5")) == True:
                ...
                print(bcolors.GREEN +
                      ...
                      + bcolors.ENDC)
                # print(trajectory['timestamp'].shape)
                for i in tqdm(range(nFrames)):
                    gotImage, frame = cap.read()
                    cnt += 1
                    #print(cnt)
                    if gotImage:
                        if cnt % 10 == 0:  # reduce to 10Hz
            ...
            if (os.path.isfile(config.save_path+"cam/" + str(videoid) + "_" + str(vidName) + ".h5")) == False:
                ...
                dset = cam.create_dataset("/X", data=frames, chunks=(
                    config.chunksize, 384 * scalefactor, 640 * scalefactor, 3), dtype='uint8')
            else:
                print(bcolors.GREEN +
